...
module.exports = function appLoader(content) {
  ...
  let configCode = `module.exports = ${JSON.stringify(config, null, 2)}`;
  let cssCode = transformed.style ? defaultStyle + transformed.style : defaultStyle;
  if (mode === 'build') {
  ...
  writeFileSync(join(outputPath, 'app.config.js'), configCode);
  const appCssPath = join(outputPath, 'app' + platform.extension.css);
  writeFileSync(appCssPath, cssCode);

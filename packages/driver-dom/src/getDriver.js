...
function getDriver(_document) {
    ...
    if (isHydrating) {
      ...
      if (hydrationChild) {
        if (hydrationChild.nodeType === COMMENT_NODE) {
          ...
          node = _document[CREATE_COMMENT](EMPTY);
    ...
    if (isHydrating) {
      ...
      if (hydrationChild) {
        if (hydrationChild.nodeType === TEXT_NODE) {
          ...
          node = _document[CREATE_TEXT_NODE](text);
          replaceChild(node, hydrationChild, parent);
        }
      } else {
        node = _document[CREATE_TEXT_NODE](text);
  ...
  */
  function createElement(type, props, component, __shouldConvertUnitlessToRpx) {
    ...
    function createNode() {
      if (isSVGMode) {
        node = _document.createElementNS(SVG_NS, type);
      } else if (tagNamePrefix) {
        let tagNamePrefix = typeof tagNamePrefix === 'function' ? tagNamePrefix(type) : tagNamePrefix;
        node = _document[CREATE_ELEMENT](tagNamePrefix + type);
      } else {
        node = _document[CREATE_ELEMENT](type);
      }
    }

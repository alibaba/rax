...
module.exports = function transPageConfig(config, dependencies = {}) {
   ...
   */
  const beforeUpdate = typeof config.beforeUpdate === 'function'
    ? config.beforeUpdate
    : noop;
  const updated = typeof config.updated === 'function'
    ? config.updated
    : noop;
  ...
  for (let key in dependencies) {
    if (dependencies.hasOwnProperty(key)) {
       ...
       */
      const beforeUpdate = typeof tplConfig.beforeUpdate === 'function'
        ? tplConfig.beforeUpdate
        : noop;
      const updated = typeof tplConfig.updated === 'function'
        ? tplConfig.updated
        : noop;
